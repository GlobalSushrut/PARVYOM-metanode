//! ENC Cluster Endpoints with JWT Security (Simplified)
//! 
...
/// ENC Cluster Endpoint Manager (Simplified)
#[derive(Debug)]
...
impl EncClusterEndpoints {
    ...
    pub fn create_enc_app_endpoint(&mut self, bind_address: SocketAddr) -> Result<String> {
        ...
        self.vpods.insert(vpod_id.clone(), vpod);
        
        println!("ENC App endpoint created: {vpod_id} on {bind_address}");
        Ok(vpod_id)
    }

    /// Create ENC BPCI Mesh endpoint (owner-only access for mainnet)
    pub fn create_enc_bpci_mesh_endpoint(&mut self, bind_address: SocketAddr) -> Result<String> {
        ...
        println!("ENC BPCI Mesh endpoint created: {vpod_id} on {bind_address}");
        Ok(vpod_id)
    }

    /// Create App Runner/Validator endpoint
    pub fn create_app_validator_endpoint(&mut self, bind_address: SocketAddr) -> Result<String> {
        ...
        println!("App Validator endpoint created: {vpod_id} on {bind_address}");
        Ok(vpod_id)
    }

    /// Create BPCI Logic endpoint for network control
    pub fn create_bpci_logic_endpoint(&mut self, bind_address: SocketAddr) -> Result<String> {
        ...
        println!("BPCI Logic endpoint created: {vpod_id} on {bind_address}");
        Ok(vpod_id)
    }

    /// Create High-Throughput/Community endpoint
    pub fn create_high_throughput_endpoint(&mut self, bind_address: SocketAddr) -> Result<String> {
        ...
        println!("High-Throughput endpoint created: {vpod_id} on {bind_address}");
        Ok(vpod_id)
    }

    /// Generate JWT token for endpoint access
    pub fn generate_jwt_token(&mut self, endpoint_type: EndpointType, subject: &str) -> Result<String> {
        ...
        let token = format!("jwt-{}-{}-{}", subject, self.config.cluster_id, now);
        
        // Store active token
        self.active_tokens.insert(token.clone(), payload);
        
        println!("JWT token generated for {subject} on endpoint {endpoint_type:?}");
        Ok(token)
    }

    /// Validate JWT token
    pub fn validate_jwt_token(&self, token: &str) -> Result<AuthResult> {
        if let Some(payload) = self.active_tokens.get(token) {
            let now = SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs();
    ...
    pub fn get_cluster_endpoints(&self) -> Vec<(String, EndpointType, SocketAddr)> {
        self.vpods.values()
    ...
    pub fn is_owner_only_endpoint(&self, vpod_id: &str) -> bool {
        if let Some(vpod) = self.vpods.get(vpod_id) {
    ...
    pub fn setup_complete_cluster(&mut self, base_port: u16) -> Result<HashMap<EndpointType, String>> {
        let mut endpoints = HashMap::new();
        
        // ENC App endpoint (public)
        let enc_app_addr = format!("127.0.0.1:{base_port}").parse()?;
        let enc_app_id = self.create_enc_app_endpoint(enc_app_addr)?;
        ...
        let enc_bpci_id = self.create_enc_bpci_mesh_endpoint(enc_bpci_addr)?;
        ...
        let app_validator_id = self.create_app_validator_endpoint(app_validator_addr)?;
        endpoints.insert(EndpointType::AppValidator, app_validator_id);
        
        // BPCI Logic endpoint
        let bpci_logic_addr = format!("127.0.0.1:{}", base_port + 3).parse()?;
        let bpci_logic_id = self.create_bpci_logic_endpoint(bpci_logic_addr)?;
        ...
        let high_throughput_id = self.create_high_throughput_endpoint(high_throughput_addr)?;
        endpoints.insert(EndpointType::HighThroughput, high_throughput_id);
        
        println!("Complete cluster setup with {} endpoints", endpoints.len());
        Ok(endpoints)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_enc_cluster_endpoints_creation() {
        let config = create_test_cluster_config();
        let endpoints = EncClusterEndpoints::new(config);
        
        let cluster_endpoints = endpoints.get_cluster_endpoints();
    ...
    #[test]
    fn test_enc_app_endpoint_creation() {
        let config = create_test_cluster_config();
        let mut endpoints = EncClusterEndpoints::new(config);
        
        let bind_addr = "127.0.0.1:8080".parse().unwrap();
        let vpod_id = endpoints.create_enc_app_endpoint(bind_addr).unwrap();
    ...
    #[test]
    fn test_enc_bpci_mesh_endpoint_creation() {
        let config = create_test_cluster_config();
        let mut endpoints = EncClusterEndpoints::new(config);
        
        let bind_addr = "127.0.0.1:8081".parse().unwrap();
        let vpod_id = endpoints.create_enc_bpci_mesh_endpoint(bind_addr).unwrap();
        
        assert!(vpod_id.starts_with("enc-bpci-test-cluster-1"));
        
        let is_owner_only = endpoints.is_owner_only_endpoint(&vpod_id);
    ...
    fn test_jwt_token_generation_and_validation() {
        ...
        let token = endpoints.generate_jwt_token(EndpointType::EncApp, "user-123").unwrap();
        assert!(!token.is_empty());
        
        let auth_result = endpoints.validate_jwt_token(&token).unwrap();
    ...
    #[test]
    fn test_complete_cluster_setup() {
        let config = create_test_cluster_config();
        let mut endpoints = EncClusterEndpoints::new(config);
        
        let cluster_endpoints = endpoints.setup_complete_cluster(9000).unwrap();
    ...
    #[test]
    fn test_enc_cluster_endpoints_exit_criteria() {
        ...
        let enc_app_id = endpoints.create_enc_app_endpoint(enc_app_addr).unwrap();
        ...
        let enc_bpci_id = endpoints.create_enc_bpci_mesh_endpoint(enc_bpci_addr).unwrap();
        let is_owner_only = endpoints.is_owner_only_endpoint(&enc_bpci_id);
        assert!(is_owner_only);
        println!("✅ Test 2: ENC BPCI Mesh endpoint (owner-only) - PASSED");
        
        // Test 3: JWT security for all endpoints
        let token = endpoints.generate_jwt_token(EndpointType::EncBpciMesh, "owner-123").unwrap();
        let auth_result = endpoints.validate_jwt_token(&token).unwrap();
        assert!(auth_result.authenticated);
